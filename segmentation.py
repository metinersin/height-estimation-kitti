...
def parse_args() -> tuple[str, int, int, str, bool]:
    ...
    # Add arguments
    parser.add_argument('date', type=au.valid_date,
                        help='Date in YYYY_MM_DD format')
    parser.add_argument('drive', type=au.positive_int,
                        help='Drive number (non-negative integer)')
    parser.add_argument('cam', type=au.valid_cam,
                        help='Cam number (must be 0, 1, 2, or 3)')
    parser.add_argument('--prompt', type=str, default='ground',
                        help="Prompt value (default: 'ground')")
    parser.add_argument('--debug', action='store_true',
                        help="Enable debug mode to save masked images")
...
def main() -> None:
    ...
    output_folder = os.path.join(
        data_path, f'segmentation_{cam:02}', f'{prompt}')
